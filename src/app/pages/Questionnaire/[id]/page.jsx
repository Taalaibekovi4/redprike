...
import { getQuestionnaireAgentId } from "@/app/store/slice/questionnaire-get";
...
const page = (prmas) => {
  const id = prmas.params.id;
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getQuestionnaireAgentId(prmas.params.id));
  }, [prmas.params.id]);
  const data = useSelector((state) => state.questionnaireGetSlice);
  ...
  useEffect(() => {
    if (isClientListModule == true) {
    }
  }, [isClientListModule]);

  return (
    <div style={{ maxWidth: 967, margin: "0 auto", position: "relative" }}>
      <div className={s.user_idRoles}>
        <Link href="/pages/Questionnaire" className={s.user_idRoleDiv}>
